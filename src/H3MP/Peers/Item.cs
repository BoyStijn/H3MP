...
{
	public class Item : IRenderUpdatable, IDisposable
	{
		...
		private readonly Snapshots<ItemTransformsMessage> _snapshots;
		...
		internal Item(ManualLogSource log, ClientPuppetConfig config, Func<ServerTime> timeGetter, double tickDeltaTime)
		{
			_log = log;
			...
			var killer = new TimeSnapshotKiller<ItemTransformsMessage>(() => Time.Now, 5);
			_snapshots = new Snapshots<ItemTransformsMessage>(killer);
		...
		public void ProcessTransforms(Timestamped<ItemTransformsMessage> message)
		{
			var time = Time;
			if (!(time is null))
			{
		...
		public void RenderUpdate()
		{
			...
			snapshot.Item.Apply(_body.transform);
		}
